...
- reducible over F : 인수분해 가능
- irreducible over F : 인수분해 불가능
- prime polynomial : 상수가 아닌 irreducible polynomial
- algebraically closed : 어떤 체 $\mathbb{F}$가 algebraically closed이려면 prime polynomial의 차수가 모두 1이어야 한다.  
  예시 : 복소수는 algebraically closed이지만 실수는 algebraically closed가 아님
...
fg를 나누는 prime polynomial p가 있으면 그것은 f, g 둘중 적어도 하나를 나눈다. 
> prime polynomial의 정의에 의해 잘 나누다 보면 f, g를 prime polynomial로 분해할 수 있다.  
> 그러면 prime polynomial의 정의에 의해 fg prime polynomial 중 하나가 p여야 한다.  
> 그러면 둘중 하나를 나눈다.  
> 바로 아래의 정리를 사용하여 스칼라배를 먼저 빼준 뒤 유일한 prime polynomial 표현 있다고 생각하면 된다.  

<br>

monic polynomial은 monic prime polynomial들로 유일하게 나누어진다(순서 제외)
> f가 1차면 당연히 기약이다. 이것 자체로 유일한 monic polynomial의 곱이다.  
> n차 이하에서 모두 유일하게 표현된다고 하면 n+1차에서도 유일하게 표현됨을 보이면 된다.  
> n+1차 다항식 f가 기약이라면 그냥 f로 유일하게 표현  
> 아니라면 g, h로 나누어지게 되어 g, h는 1차 이상의 monic polynomial이다.  
> 이때 g, h는 모두 유일하게 표현된다.  

<br>

$f$가 인수분해가 되지 않는 서로 다른 다항식들의 곱으로 나타난다는 것과 $f$, $f^{'}$가 서로소인 것은 동치이다. 
> 만약 서로 다른 다항식의 곱이 아니라 같은 다항식의 곱으로 나타난다면 $f=p^2h$, $f^{'}=p(2p^{'}h+ph^{'})$ 서로소 아니다.  
...
- n-linear function : D가 n*n 행렬에서 스칼라로 가는 선형변환일 때 모든 i에 대해 다른 행을 고정하고 i번째 행만 바뀔 때 그 함수가 선형이면 n-linear이다. 
- alternating : n-linear function에서 두 행이 같을 때 0이고, 어떤 두 행이 바뀌면 부호도 바뀌는 것을 alternate, alternating이라고 한다.  
  근데 두 행이 같을때 0이라는 조건만 있어도 바뀌면 부호 반대 조건을 얻어낼 수 있다.  
...
![img_2.png](img_2.png)  
> n-linear, alternating, I 넣으면 1 3개만 증명하면 된다.  
> $D_{ij}$가 (n-1)-linear이라서 행 하나가 스칼라배 되면 전체 그 i에서 스칼라배, 다른 i에서도 다 같은 스칼라배로 n-linear  
...
$$
$$
D(A)=D(a_1, a_2, ..., a_n)=\sum_{k=1}^n (A_{1k}D(\epsilon_k, a_2, ..., a_n))
$$
$$
\text{같은 거 있으면 0이 된다} \rightarrow \text{모두 다른 permutation}\ \sigma_i\text{들이 존재한다.}  
$$
$$
\sum_{\sigma} (A_{1\sigma_1}A_{1\sigma_2}...A_{1\sigma_n}D(\epsilon_{\sigma_1}, \epsilon_{\sigma_2}, ..., \epsilon_{\sigma_n}))
$$
$$
D(\epsilon_{\sigma_1}, \epsilon_{\sigma_2}, ..., \epsilon_{\sigma_n})=±1=sgn(\sigma)
$$
